...
public class C_LinkedListTest {
	...
	/*对LinkedList的总结:
	 * 1.除了具有ArrayList的所有功能以外,LinkedList还提供了操作头尾的操作(LinkedList底层虽然是通过链表实现的,但很多方面都和数组相似)和
	 *   一些与栈有关的处理操作
	 * 2.继承与List这一接口
	 * 3.什么时候用ArrayList?什么时候用LinkedList?
	 *   与底层实现有关,要知道每一种数据结构都有其适用的范围:
	 *   底层是链表实现的适合较多的增删改操作,底层是数组实现的比较适合较多的查操作
	 * 4.非线程同步,会出现快速失败异常
	 * 5.除了有普通的单向Iterator迭代器外还有listIterator这一双向迭代器
	 */
