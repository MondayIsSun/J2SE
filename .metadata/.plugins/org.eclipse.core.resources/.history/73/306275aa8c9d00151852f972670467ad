...
public class A_student {
	 ...
	 * 又因为在调用对象的equals()方法一般都会使用到对象的哈希值进行比较...而每个对象的哈希值是唯一的,所以用哈希值进行比较两个对象相等就等同于用对象的句柄比较大小,这是达不到效果的
	 * 所以常常有必要去重写对象身上hashCode()方法...重写的原则是:对象equals必然有相等的hash值,hash值相等的对象不一定equals...
	 * 这样重写过了equals()和hashCode()方法的对象在比较时的效率也提高了...
